...
use crate::models::world::StoredGateJumped;
...
impl GateJumpedHanlder {
    ...
    fn is_gate_jumped(&self, event: &Event) -> bool {
        let module_name = "gate";
        let event_name = "JumpEvent";
    ...
    async fn process(&self, checkpoint: &Arc<Checkpoint>) -> anyhow::Result<Vec<Self::Value>> {
        ...
        for tx in &checkpoint.transactions {
            ...
            for (index, ev) in events.data.iter().enumerate() {
                if self.is_gate_jumped(ev) {
                    let meta = base_meta.with_index(index);
                    let event = StoredGateJumped::from_event(ev, &meta);
                    results.push(event);
                }
            }
        }
...
impl Handler for GateJumpedHanlder {
    ...
    async fn commit<'a>(
        ...
        use crate::schema::indexer::events_gate_jumped::dsl::*;

        diesel::insert_into(events_gate_jumped)
            .values(batch)
