...
use crate::models::world::StoredTurretCreated;

use crate::AppContext;

pub struct TurretCreatedHandler {
    ctx: AppContext,
    package_set: HashSet<AccountAddress>,
}

impl TurretCreatedHandler {
    ...
    fn is_turret_created(&self, event: &Event) -> bool {
        let module_name = "turret";
        let event_name = "TurretCreatedEvent";
...
#[async_trait]
impl Processor for TurretCreatedHandler {
    const NAME: &'static str = "turret_created";
    type Value = StoredTurretCreated;
    ...
    async fn process(&self, checkpoint: &Arc<Checkpoint>) -> anyhow::Result<Vec<Self::Value>> {
        ...
        for tx in &checkpoint.transactions {
            ...
            for (index, ev) in events.data.iter().enumerate() {
                if self.is_turret_created(ev) {
                    let meta = base_meta.with_index(index);
                    let event = StoredTurretCreated::from_event(ev, &meta);
                    results.push(event);
                }
            }
        }
...
#[async_trait]
impl Handler for TurretCreatedHandler {
    type Store = Db;
