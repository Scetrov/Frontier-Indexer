...
#[async_trait]
impl Processor for FuelBurningStoppedHandler {
    const NAME: &'static str = "fuel_burning_stopped";
    type Value = StoredFuelBurningStopped;
    ...
    async fn process(&self, checkpoint: &Arc<Checkpoint>) -> anyhow::Result<Vec<Self::Value>> {
        ...
        for tx in &checkpoint.transactions {
            ...
            for (index, ev) in events.data.iter().enumerate() {
                if !self.is_fuel_burning_stopped(ev) {
                    continue;
                }
                ...
                if !matches!(move_event.action, MoveFuelAction::BurningStopped) {
                    continue;
                }

                let meta = base_meta.with_index(index);
                let event = StoredFuelBurningStopped::from_event(&move_event, &meta);
                results.push(event);
            }
        }
...
#[async_trait]
impl Handler for FuelBurningStoppedHandler {
    type Store = Db;
    ...
    async fn commit<'a>(
        ...
        use crate::schema::indexer::events_fuel_burning_stopped::dsl::*;

        diesel::insert_into(events_fuel_burning_stopped)
            .values(batch)
