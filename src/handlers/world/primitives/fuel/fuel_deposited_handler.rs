...
#[async_trait]
impl Processor for FuelDepositedHandler {
    const NAME: &'static str = "fuel_deposited";
    type Value = StoredFuelDeposited;
    ...
    async fn process(&self, checkpoint: &Arc<Checkpoint>) -> anyhow::Result<Vec<Self::Value>> {
        ...
        for tx in &checkpoint.transactions {
            ...
            for (index, ev) in events.data.iter().enumerate() {
                ...
                if !matches!(move_event.action, MoveFuelAction::Deposited) {
                    continue;
                }

                let meta = base_meta.with_index(index);
                let event = StoredFuelDeposited::from_event(&move_event, &meta);
                results.push(event);
            }
        }
...
#[async_trait]
impl Handler for FuelDepositedHandler {
    type Store = Db;
