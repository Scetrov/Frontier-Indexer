...
#[async_trait]
impl Processor for FuelBurningStartedHandler {
    const NAME: &'static str = "fuel_burning_started";
    type Value = StoredFuelBurningStarted;
    ...
    async fn process(&self, checkpoint: &Arc<Checkpoint>) -> anyhow::Result<Vec<Self::Value>> {
        ...
        for tx in &checkpoint.transactions {
            ...
            for (index, ev) in events.data.iter().enumerate() {
                if !self.is_fuel_burning_started(ev) {
                    continue;
                }
                ...
                if !matches!(move_event.action, MoveFuelAction::BurningStarted) {
                    continue;
                }

                let meta = base_meta.with_index(index);
                let event = StoredFuelBurningStarted::from_event(&move_event, &meta);
                results.push(event);
            }
        }
...
#[async_trait]
impl Handler for FuelBurningStartedHandler {
    type Store = Db;
